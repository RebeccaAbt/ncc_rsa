...
class Fusion_CP(Job):
    def run(self,
            ...
            thresholds = [0.05],     # for defnition of good clusters after cluster permutation
            meanMEG = False,
            tail = 1, # tail = 2
        ...
        if meanMEG: # check if this is correct logic!!!! added this as a fix after a long break so I need to review, if this is correct hoere, oir if there was a reason, why ['fusion_mean_pkl'] was always loaded here
        ...
        for modality  in modalities:
            ...
            cluster_def_thres   = get_cluster_def_thres(data_pre_noNan, method = cluster_def_method, tail = tailArg) 
            mask                = find_voxels_noNan_allSubj(all_data_pre, all_data_post, cfg.get_mask_file())
            spatial_adjacency   = get_spatial_adjacency_3d(mask) # must be the mask that is also used to mask the data, because adjacency is computed for those voxels that are True in the mask
            ...
            for X_def in cp_variables:
                ...
                T_obs, clusters, cluster_pv, H0 = clu = \
                    cluster_permutation(X, cluster_def_thres, spatial_adjacency, tail = tailArg, n_jobs=8) # shape of T_obs: (n_times, n_voxels)
            

                print(f'\t [4] Saving clusters...', flush=True)

                # ------------------------ added different File ending!!!
                # file_note_clu = f'{n_subj}_subj_{modality}' # anottate number of subjects & thresold used for defining "good" clusters
                # file_note_clu = f'{n_subj}_subj_{modality}_cDef_p' # anottate number of subjects & thresold used for defining "good" clusters

                file_note_clu = f'{n_subj}_subj_{modality}_{X_def}_{cluster_def_method}Thres_{tail}tailed'
                ...
                print(f'\t [5] Saving good clusters...', flush=True)
                ...
                for thres in thresholds:
                    ...
                    # file_note_good_clu = f'{n_subj}_subj_thres_{thres_str}_{modality}_cDef_p'

                    file_note_good_clu = f'{n_subj}_subj_{modality}_{X_def}_{cluster_def_method}Thres_{tail}tailed_thres_{thres_str}'
                    # ------------------------ 
