...
from utils.rsa_meg import create_run_idx, count_events_per_run
...
class RDMmovie(Job):
    def run(self,
        ...
        epochs = (joblib.load(MEG_file)['epochs_meg']
                    .filter(picks='bio', l_freq=None, h_freq=30, n_jobs=-1)
                    #.resample(100)
                    )

        count_events_per_run(epochs, subjectID) # <-------------------------------------- This is new! Need to check if it works!
        ...
        modality = np.array([event_name.split('/')[0] for event_name in event_names]) #auditory, somato & visual
        hits = np.array([event_name.split('/')[1] for event_name in event_names]) #hits 
        stim_type = np.array([event_name.split('/')[2] for event_name in event_names]) #stim numbers..
        ...
        rdm_movie = rsa.rdm.calc_rdm_movie_parallel(
                                data,
