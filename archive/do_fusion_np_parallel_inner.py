...
class Fusion_np_inner(Job):
    def run(self,
        ...
        elif not meanMEG and not os.path.exists(outFiles['fusion_pkl']): # stop the execution here, if the output file already exists
            msg = (
                '---------------------------------------------------------------------------------------\n'
        ...
        # Prepare arguments
        args = [(t, SL_rdms, sensory_vec, suprasensory_vec) for t in rdm_movie]

        # Run in parallel:
        # Do the fusion by computing the commonality coefficients of both models
        fusion_data = []
        for t in tqdm(rdm_movie, desc='Fusion fMRI and MEG data for timepoint...'):
 
           fusion_data.append(fuse_timepoint_np_parallel(t, SL_rdms, sensory_vec, suprasensory_vec))
